...
#st.write('')
#st.write('')
st.title("Reverse Image Search via Vector Database + LLM Summary")
...
def plot_similar_images_new(image_path, text_input, number_of_images: int = 6):
	...
	if openai_api_key != "":		
		...
		llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash-exp", temperature=0.7, max_tokens=None, timeout=None, max_retries=2, google_api_key=openai_api_key)
